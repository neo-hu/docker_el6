...
import (
	...
	"github.com/neo-hu/docker_el6/cmd/runc/libcontainer/criurpc"
	"github.com/neo-hu/docker_el6/cmd/runc/libcontainer/intelrdt"
	"github.com/neo-hu/docker_el6/cmd/runc/libcontainer/system"
...
func (c *linuxContainer) includeExecFifo(cmd *exec.Cmd) error {
	fifoName := filepath.Join(c.root, execFifoFilename)
	//fifoFd, err := unix.Open(fifoName, unix.SOCK_NONBLOCK| unix.O_PATH|unix.O_CLOEXEC, 0)
	...
	cmd.Env = append(cmd.Env,
		fmt.Sprintf("_LIBCONTAINER_FIFO=%s", fifoName))
	return nil
}
